...
-- Hàm setup Smooth Scrolling cho ScrollingFrame (Touch/Mouse)
function UtilsModule:SetupSmoothScroll(scrollFrame)
    local scrolling = false
    local lastTouchPos = Vector2.new(0, 0)
    local velocity = 0
    local inertiaConnection = nil
    
    -- Hàm áp dụng inertia scroll
    local function applyInertia()
        ...
        inertiaConnection = RunService.Heartbeat:Connect(function()
            if scrolling then
                inertiaConnection:Disconnect()
                inertiaConnection = nil
                return
            ...
            local newPos = scrollFrame.CanvasPosition.Y + velocity
            newPos = math.clamp(newPos, 0, scrollFrame.CanvasSize.Y.Offset - scrollFrame.AbsoluteSize.Y)
            scrollFrame.CanvasPosition = Vector2.new(0, newPos)
        end)
        
        self:RegisterConnection(inertiaConnection, "SmoothScroll")
    end
    ...
    scrollFrame.InputChanged:Connect(function(input)
        ...
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseMovement then
            ...
            local delta = lastTouchPos.Y - currentPos.Y
            velocity = delta
            lastTouchPos = currentPos
            
            local newPos = scrollFrame.CanvasPosition.Y + delta
            newPos = math.clamp(newPos, 0, math.max(0, scrollFrame.CanvasSize.Y.Offset - scrollFrame.AbsoluteSize.Y))
            scrollFrame.CanvasPosition = Vector2.new(0, newPos)
        end
    end)
    ...
    scrollFrame.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseWheel then
            local scrollAmount = -input.Position.Z * 40 -- Adjust scroll speed
            local newPos = scrollFrame.CanvasPosition.Y + scrollAmount
            newPos = math.clamp(newPos, 0, math.max(0, scrollFrame.CanvasSize.Y.Offset - scrollFrame.AbsoluteSize.Y))
            
            -- Smooth tween scroll
            TweenService:Create(scrollFrame, TweenInfo.new(0.15, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
...
-- Tạo Main Frame (Kích thước nhỏ gọn hơn: 320x420)
...
-- Setup Dragging Ultra Smooth (Hỗ trợ cả Mouse và Touch, không giật lag)
function UIModule:SetupDragging(dragHandle)
    ...
    -- Hàm cập nhật vị trí trực tiếp (không dùng Tween để tránh lag)
    local function updatePosition(inputPos)
        ...
        local screenSize = self.Elements.ScreenGui.AbsoluteSize
        local frameSize = self.Elements.MainFrame.AbsoluteSize
        newX = math.clamp(newX, -frameSize.X + 50, screenSize.X - 50)
        newY = math.clamp(newY, 0, screenSize.Y - 50)
        
        self.Elements.MainFrame.Position = UDim2.new(0, newX, 0, newY)
    end
    
    -- Hàm kiểm tra click vào nút
    local function isClickOnButton(inputPos)
        local minimizeBtn = self.Elements.MinimizeBtn
        local closeBtn = self.Elements.CloseBtn
        
        if not minimizeBtn or not closeBtn then return false end
        
        local guiInset = game:GetService("GuiService"):GetGuiInset()
        local adjustedPos = inputPos - guiInset
        
        local function isInBounds(btn)
            local pos = btn.AbsolutePosition
            local size = btn.AbsoluteSize
            return adjustedPos.X >= pos.X and adjustedPos.X <= pos.X + size.X
                and adjustedPos.Y >= pos.Y and adjustedPos.Y <= pos.Y + size.Y
        end
        
        return isInBounds(minimizeBtn) or isInBounds(closeBtn)
    end
    
    -- Inertia/momentum sau khi thả (vuốt mượt)
    local function applyInertia()
        if inertiaConnection then
            inertiaConnection:Disconnect()
            inertiaConnection = nil
        ...
        inertiaConnection = RunService.Heartbeat:Connect(function()
            ...
            if velocity.Magnitude < minVelocity then
                inertiaConnection:Disconnect()
                inertiaConnection = nil
                return
            end
            
            local currentPos = self.Elements.MainFrame.Position
            local screenSize = self.Elements.ScreenGui.AbsoluteSize
            local frameSize = self.Elements.MainFrame.AbsoluteSize
            
            local newX = math.clamp(currentPos.X.Offset + velocity.X, -frameSize.X + 50, screenSize.X - 50)
            local newY = math.clamp(currentPos.Y.Offset + velocity.Y, 0, screenSize.Y - 50)
            
            self.Elements.MainFrame.Position = UDim2.new(0, newX, 0, newY)
        end)
        
        table.insert(ConfigModule.State.ActiveConnections, inertiaConnection)
    end
    
    -- Input Began (Mouse hoặc Touch)
    dragHandle.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            -- Kiểm tra không click vào nút
            if isClickOnButton(input.Position) then
                return
            end
            
            -- Dừng inertia nếu đang chạy
            if inertiaConnection then
                inertiaConnection:Disconnect()
                inertiaConnection = nil
            end
            ...
            ConfigModule.State.IsDragging = true
        end
    end)
    
    -- Global Input Changed (để tracking mượt hơn)
    local inputChangedConn = UserInputService.InputChanged:Connect(function(input)
        if not dragging then return end
        
        if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
            local currentPos = Vector2.new(input.Position.X, input.Position.Y)
            velocity = currentPos - lastInputPos
            lastInputPos = currentPos
            updatePosition(currentPos)
        end
    end)
    table.insert(ConfigModule.State.ActiveConnections, inputChangedConn)
    ...
    local inputEndedConn = UserInputService.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            if dragging then
                dragging = false
                ConfigModule.State.IsDragging = false
                
                -- Áp dụng inertia nếu có velocity
    ...
    table.insert(ConfigModule.State.ActiveConnections, inputEndedConn)
end
...
function UIModule:CreateLagReductionTab()
    ...
    lagTab.ScrollBarThickness = 3
    lagTab.ScrollBarImageColor3 = self.Theme.Accent
    lagTab.ScrollBarImageTransparency = 0.3
    lagTab.CanvasSize = UDim2.new(0, 0, 0, 380)
    lagTab.ScrollingDirection = Enum.ScrollingDirection.Y
    lagTab.ElasticBehavior = Enum.ElasticBehavior.Never
    lagTab.Visible = true
    lagTab.Active = true
    ...
    UtilsModule:SetupSmoothScroll(lagTab)
...
function UIModule:CreateMemoryTab()
    ...
    memoryTab.ScrollBarThickness = 3
    memoryTab.ScrollBarImageColor3 = self.Theme.Accent
    memoryTab.ScrollBarImageTransparency = 0.3
    memoryTab.CanvasSize = UDim2.new(0, 0, 0, 340)
    memoryTab.ScrollingDirection = Enum.ScrollingDirection.Y
    memoryTab.ElasticBehavior = Enum.ElasticBehavior.Never
    ...
    -- Setup smooth scrolling
    UtilsModule:SetupSmoothScroll(memoryTab)
...
function UIModule:CreateProfilesTab()
    ...
    profilesTab.ScrollBarThickness = 3
    profilesTab.ScrollBarImageColor3 = self.Theme.Accent
    profilesTab.ScrollBarImageTransparency = 0.3
    profilesTab.CanvasSize = UDim2.new(0, 0, 0, 310)
    profilesTab.ScrollingDirection = Enum.ScrollingDirection.Y
    profilesTab.ElasticBehavior = Enum.ElasticBehavior.Never
    profilesTab.Visible = false
    profilesTab.Active = true
    profilesTab.Parent = self.Elements.ContentContainer
    
    self.Elements.ProfilesContent = profilesTab
    
    -- Setup smooth scrolling
    UtilsModule:SetupSmoothScroll(profilesTab)
...
-- Setup Mini Toolbar Dragging (Ultra Smooth với Inertia)
function UIModule:SetupMiniDragging(toolbar)
    ...
    local velocity = Vector2.new(0, 0)
    local inertiaConnection = nil
    local dragThreshold = 5 -- pixels để phân biệt click và drag
    local totalDragDistance = 0
    
    -- Hàm cập nhật vị trí trực tiếp
    local function updatePosition(inputPos)
        local delta = inputPos - dragStart
        local screenSize = self.Elements.ScreenGui.AbsoluteSize
        
        local newX = math.clamp(startPos.X.Offset + delta.X, 5, screenSize.X - 60)
        local newY = math.clamp(startPos.Y.Offset + delta.Y, 5, screenSize.Y - 60)
    ...
    local function applyInertia()
        ...
        inertiaConnection = RunService.Heartbeat:Connect(function()
            ...
            local currentPos = toolbar.Position
            local screenSize = self.Elements.ScreenGui.AbsoluteSize
            
            local newX = math.clamp(currentPos.X.Offset + velocity.X, 5, screenSize.X - 60)
            local newY = math.clamp(currentPos.Y.Offset + velocity.Y, 5, screenSize.Y - 60)
    ...
    local inputEndedConn = UserInputService.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            if dragging then
                ...
                -- Nếu chỉ click (không drag nhiều), thì expand UI
                if totalDragDistance < dragThreshold then
                    self:ExpandUI()
                elseif velocity.Magnitude > 1.5 then
                    -- Nếu có velocity, áp dụng inertia
                    applyInertia()
                end
            end
        end
...
-- Minimize UI (Thu nhỏ thành icon nổi - Animation siêu mượt)
function UIModule:MinimizeUI()
    if ConfigModule.State.IsMinimized then return end
    ...
    self.LastMainFrameSize = self.Elements.MainFrame.Size
    
    -- Tính toán vị trí center của MainFrame
    local mainPos = self.Elements.MainFrame.AbsolutePosition
    local mainSize = self.Elements.MainFrame.AbsoluteSize
    local centerX = mainPos.X + mainSize.X / 2
    local centerY = mainPos.Y + mainSize.Y / 2
    
    -- Đặt vị trí MiniToolbar tại center của MainFrame
    self.Elements.MiniToolbar.Position = UDim2.new(0, centerX - 27, 0, centerY - 27)
    self.Elements.MiniToolbar.Size = UDim2.new(0, 0, 0, 0)
    self.Elements.MiniToolbar.Visible = true
    
    -- Animation thu nhỏ MainFrame (nhanh và mượt)
    local shrinkTween = TweenService:Create(self.Elements.MainFrame, TweenInfo.new(0.2, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
        Size = UDim2.new(0, 0, 0, 0),
        Position = UDim2.new(0, centerX, 0, centerY)
    })
    
    -- Animation hiện MiniToolbar (pop effect)
    local showMiniTween = TweenService:Create(self.Elements.MiniToolbar, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
        Size = UDim2.new(0, 55, 0, 55)
    })
    ...
    shrinkTween.Completed:Connect(function()
        ...
        -- Reset MainFrame size cho lần expand sau
        self.Elements.MainFrame.Size = UDim2.new(0, 320, 0, 420)
        self.Elements.MainFrame.Position = self.LastMainFramePosition
    ...
    -- Chạy cả 2 animation song song
    shrinkTween:Play()
    showMiniTween:Play()
end

-- Expand UI (Mở rộng từ icon thành menu đầy đủ - Animation siêu mượt)
function UIModule:ExpandUI()
    if not ConfigModule.State.IsMinimized then return end
    ...
    -- Lấy vị trí center của MiniToolbar
    local miniPos = self.Elements.MiniToolbar.AbsolutePosition
    local miniSize = self.Elements.MiniToolbar.AbsoluteSize
    local centerX = miniPos.X + miniSize.X / 2
    local centerY = miniPos.Y + miniSize.Y / 2
    
    -- Tính vị trí target cho MainFrame (centered từ mini position)
    local targetPos = self.LastMainFramePosition or UDim2.new(0, centerX - 160, 0, centerY - 210)
    
    -- Giới hạn trong màn hình
    local screenSize = self.Elements.ScreenGui.AbsoluteSize
    local targetX = math.clamp(targetPos.X.Offset, 10, screenSize.X - 330)
    local targetY = math.clamp(targetPos.Y.Offset, 10, screenSize.Y - 430)
    targetPos = UDim2.new(0, targetX, 0, targetY)
    
    -- Chuẩn bị MainFrame
    self.Elements.MainFrame.Size = UDim2.new(0, 0, 0, 0)
    self.Elements.MainFrame.Position = UDim2.new(0, centerX, 0, centerY)
    self.Elements.MainFrame.Visible = true
    self.Elements.MainFrame.BackgroundTransparency = ConfigModule.Settings.UITransparency
    
    -- Animation thu nhỏ MiniToolbar
    local hideMiniTween = TweenService:Create(self.Elements.MiniToolbar, TweenInfo.new(0.15, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
        Size = UDim2.new(0, 0, 0, 0)
    })
    
    -- Animation mở rộng MainFrame (pop effect)
    local expandTween = TweenService:Create(self.Elements.MainFrame, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
        Size = UDim2.new(0, 320, 0, 420),
        Position = targetPos
    })
    
    hideMiniTween.Completed:Connect(function()
        self.Elements.MiniToolbar.Visible = false
        -- Reset MiniToolbar size
        self.Elements.MiniToolbar.Size = UDim2.new(0, 55, 0, 55)
    end)
